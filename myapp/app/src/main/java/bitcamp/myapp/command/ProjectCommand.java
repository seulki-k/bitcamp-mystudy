...
public class ProjectCommand {
    ...
    private static void updateProject() {
        ...
        Project project = projects[projectNo - 1];
        project.setTitle(Prompt.input("프로젝트명(%s)\n", project.getTitle()));
        project.setDescription(Prompt.input("설명(%s)\n", project.getDescription()));
        project.setStartDate(Prompt.input("시작일(%s)\n", project.getStartDate()));
        project.setEndDate(Prompt.input("종료일(%s)\n", project.getEndDate()));
    ...
    private static void deleteProject() {
        int projectNo = Prompt.inputInt("회원 번호?");
        if (projectNo < 1 || projectNo > projectLength) {
